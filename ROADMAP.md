...
- Toda alteração via feature branch + PR (squash and merge). Sem push direto em `main`.
...
- [ ] **1.1** BaseEntity + enums (`AppointmentStatus`, `UserRole`) + interfaces de repositório. **Spec ref: 4**
- [ ] **1.2** EF Core DbContext + mappings de todas as entidades + Global Query Filters (`TenantId`, `IsDeleted`) + índice filtrado UNIQUE `(DoctorId, StartTime)` + primeira migration. **Spec ref: 3.2, 4, 9**
