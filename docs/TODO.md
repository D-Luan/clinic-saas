...
- [ ] Os testes de smoke em `tests/HealthBr.UnitTests/SmokeTests.cs` são placeholders triviais do scaffold; substituir por testes reais quando surgir lógica de domínio (o placeholder de `tests/HealthBr.IntegrationTests/` foi substituído pelos testes de persistência da task 1.2).
...
- [ ] `RefreshToken.Token` não tem índice de lookup (task 1.2 criou apenas os índices exigidos: `Appointment(DoctorId, StartTime)` e `User(TenantId, Email)` filtrados). Login/refresh (task 1.3) consultam por hash do token — adicionar índice único em `Token` na migration da própria task 1.3.
